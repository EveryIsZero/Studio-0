...
- **🎨 免代码（No‑Code）**：**图形化界面操作**——选择 COM 端口一键连接、标签页切换数据区域、滑块调参、点击发送指令。全程无需写一行代码。
...
> 💡 **Studio 0（「0号工坊」）的主界面** —— 所有功能集成在一个窗口中，无需编写代码即可完成 Modbus 通讯调试与数据绑定。

### 主界面实拍

下图是 Studio 0 启动后的真实界面（**输入/输出绑定 — 位操作** 标签页）：

<p align="center">
  <img src="screenshots/main_interface.png" width="900" alt="Studio 0 主界面 - 0号工坊"/>
  <br/><em>▲ 主界面全景：菜单栏 · 设备连接面板（左）· 输入/输出绑定工作区（中）· 状态栏（底）</em>
</p>
...
| 区域 | 功能 | 操作方式 |
|------|------|----------|
| **顶部菜单栏** | 文件 / 视图 / 配置 / 帮助 | 全局功能入口 |
| **工具栏** | 设备列表 · 连接 · 断开 · 高级监控 | 一键操作 |
| **左侧面板** | 设备管理：设备名称、连接/断开按钮、COM 端口选择 | 图形化串口配置 |
| **中央工作区** | 输入/输出绑定：位操作 / 线圈 / 保持寄存器（Z3Z4） | 标签页切换 |
| **数据控件** | 文本标签、滑块、配置按钮、发送按钮、进度指示 | 拖拽即可使用 |
| **位操作表** | B0–B15 ↔ Bit.16–Bit.31 寄存器映射 | 可视化位级读写 |
| **底部状态栏** | 就绪状态 / 当前串口 | 实时反馈 |

### 免代码操作流程

使用 Studio 0 **不需要写任何代码**，典型工作流如下：

1. **配置连接** — 左侧面板选择 COM 端口，点击「连接」（蓝色按钮）
2. **绑定数据** — 在中央工作区通过标签页切换**位操作 / 线圈 / 保持寄存器**
3. **调整参数** — 滑块拖动调节数值、文本框输入标签名、点击「配置」细化设置
4. **下发指令** — 点击「发送」按钮将数据写入设备，进度条实时反馈
5. **监控状态** — 位操作表实时显示每个位的当前状态（B0–B15 对应寄存器位）

### 高级监控模式

工具栏右侧提供**高级监控**入口，可切换到更详细的数据监控视图（寄存器批量查看、报文日志等）。
...
2. 找到最新版本的 `Studio 0 Release 4.3.4.69.zip` 文件
3. 点击下载（约 40 MB）
...
当前版本: **4.3.4.69** (v4.3.4.69)

| 版本 | 日期 | 说明 |
|------|------|------|
| **4.3.4.69** | 2025-06 | 首次公开发布版 |
